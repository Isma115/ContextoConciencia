...
function navigationControlsMarkup(state) {
  ...
  return `<div class="file-explorer-navigation-actions"><div class="file-explorer-history-actions" role="group" aria-label="Historial de navegación">${navigationButton('back', '←', !canGoBack, 'Atrás')}${navigationButton('forward', '→', !canGoForward, 'Adelante · volver')}</div>${navigationButton('home', 'Inicio', state.loading, 'Este equipo')}${navigationButton('parent', '↑', !canGoParent, 'Subir')}</div>`;
}
...
function breadcrumbsMarkup(state) {
  ...
  const pathMarkup = state.currentPath ? `<div class="file-explorer-breadcrumb-path">${crumbs}${state.searchActive ? '<span class="file-explorer-search-badge">Resultados</span>' : ''}</div>` : '';
  return `<nav class="file-explorer-breadcrumbs" aria-label="Ruta actual">${navigationControlsMarkup(state)}${pathMarkup}</nav>`;
}
...
function directoryMarkup(state) {
  ...
  return `<div class="file-explorer-directory-head"><div><strong>${escapeHtml(state.searchActive ? 'Resultados' : pathName(state.currentPath))}</strong><span>${escapeHtml(totalLabel)}${state.searchTruncated ? ' · búsqueda limitada' : ''}</span></div>${state.searchActive ? actionButton('clear-search', 'Limpiar', false, 'btn btn-secondary btn-small') : ''}</div><div class="file-explorer-entry-list file-explorer-entry-list-${viewMode}" role="listbox" aria-label="Contenido de la carpeta">${visibleEntries.map((entry, index) => entryMarkup(entry, index, state)).join('')}</div>`;
}
...
export function renderFileExplorerView(container, state) {
  container.innerHTML = `<div class="file-explorer-shell">${filterMarkup(state)}${breadcrumbsMarkup(state)}${selectionMarkup(state)}<main class="file-explorer-content">${bodyMarkup(state)}</main>${contextMenuMarkup(state)}</div>`;
}
