...
function modalMarkup(file, line, range = null, symbolName = '') {
  ...
  return `<div id="code-map-code-modal" class="modal-backdrop"><div class="modal code-map-code-modal" role="dialog" aria-modal="true" aria-labelledby="code-map-code-title"><div class="modal-head"><button type="button" class="modal-close" data-code-map-close-modal aria-label="Cerrar">×</button><div><h2 id="code-map-code-title">${escapeHtml(heading)}</h2><p>${escapeHtml(languageLabel(file.language))} · ${escapeHtml(file.size)} bytes${scope}</p></div></div><div class="code-map-code-body"><pre class="code-map-code-view">${visibleLines.map(({ content, lineNumber }) => `<span class="code-map-code-line${lineNumber === targetLine ? ' is-target' : ''}" data-line="${lineNumber}"><b>${String(lineNumber).padStart(numberWidth, ' ')}</b>${escapeHtml(content) || ' '}</span>`).join('')}</pre></div><div class="modal-actions"><button type="button" class="btn btn-secondary" data-copy-path="${escapeHtml(file.path)}" aria-label="Copiar ruta" title="Copiar ruta">Copiar ruta</button></div></div></div>`;
}
