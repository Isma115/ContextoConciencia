...
export function openSourceModal(existing = null, forcedType = null, initialPaths = [], initialName = '', returnView = 'sources') {
  ...
  const renderConfig = (selectedType) => {
    ...
    if (selectedType === 'local') {
      $('#source-config').querySelectorAll('[data-remove-path-index]').forEach((button) => button.addEventListener('click', () => {
        const index = Number(button.dataset.removePathIndex);
